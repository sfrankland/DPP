...
% for k=1:m;
%     for j=1:m;
%        summed(ctr,:)=locations(k,:)  + objects(j,:);
%        idx(ctr,:) = [k,j];
%        ctr=ctr+1;
%     end
% end
...
% for k=1:m;
%     for j=1:m;
%        multiplied(ctr,:)=locations(k,:) .* objects(j,:);
%        idx(ctr,:) = [k,j];
%        ctr=ctr+1;
%     end
% end
...
% for k=1:m;
%     for j=1:m;
%        tensor(ctr,:)=locations(k,:)' * objects(j,:);
%        idx(ctr,:) = [k,j];
%        ctr=ctr+1;
%     end
% end
...
% for k=1:m;
%     for j=1:m;
%        subtracted(ctr,:)=locations(k,:)  - objects(j,:);
%        idx(ctr,:) = [k,j];
%        ctr=ctr+1;
%     end
% end
...
%    Iout= triu(I)+triu(I,1)'  
  ...
  narrow_candidates = [intersect(find(idx(:, 1)==these_objs), find(idx(:, 2)==w_complement)) target_idx];
  for p=1:length(candidates);    
   ...
   [V,D]=eig(Iout);
   det_results(p)=trace(V*diag(exp(diag(D)))/V);
   %det_results(p)=det(Iout);
   %det_results(p) = det(expm(Iout));
   %det_results(p) = (expm(trace(Iout)));
   %det_results(p) = trace(expm(Iout));
  ...
  binary_sim_res(sim_val) = candidates(bres_sim)==target_idx;
  %order_sim_res(it, in_it,sim_val) = find(candidates(srtd_res)==target_idx);
  
  binary_rand(sim_val) = randsample(candidates,1)==target_idx;
