...
%    Iout= triu(I)+triu(I,1)'  
  ...
  for p=1:length(candidates);   
     list = [sub_pre candidates(p)];
     sub_output = abs(output(list, :));
     sum_sub_output = sum(sub_output, 1);
     sum_sub_output(sum_sub_output<=theta)=0;
     sum_sub_output(sum_sub_output>=theta)=1;
     det_results(p) = length(find(sum_sub_output==1));
  end
...
% % %kwta
% % for j=1:num_items;
% %     [a b] = sort(abs(sum_sub_output(j,:)), 'descend');
% %     sum_sub_output(j,b(kwta:end))=0;
% %     sum_sub_output(j,b(1:kwta))=1;
% % end
%    
...
%project into high D.
%before binarizing, normalize within context in high d.
    % have to normalize based on the assessment of the covariance!
    % number of active cells is implicitly associated w/ anti-correlation;
    % more active cells, less covariance.
%binarize. hamming distance. determinant.

%similar states will have a greater activity value. also more similar
%thresholded reps. if its not k winners, but just a threshold, could just do max active units?
