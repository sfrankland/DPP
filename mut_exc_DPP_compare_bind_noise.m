...
e=0;
for sim_val=1:1000;
clearvars -except m n e num_items binary_res order_res sim_val cc tp superpos ares1
names = randn(m,n);
c=cov(names');
...
for k=1:m;
    for j=1:m;
       circ_conv(ctr, :)=cconv(names(k,:)+randn(1,n)*e, objects(j,:)+randn(1,n)*e);
       idx(ctr,:) = [k,j];
       ctr=ctr+1;
    end
end
...
for k=1:m;
    for j=1:m;
        cur_tp = (names(k,:)+randn(1,n)*e)'*(objects(j,:)+randn(1,n)*e);
...
for k=1:m;
    for j=1:m;
       catted(ctr,:)=[(names(k,:)+randn(1,n)*e) (objects(j,:)+randn(1,n)*e)];
       idx(ctr,:) = [k,j];
       ctr=ctr+1;
    end
end
...
for k=1:m;
    for j=1:m;
       superpos_code(ctr,:)=(names(k,:)+randn(1,n)*e)+(objects(j,:)+randn(1,n)*e);
       idx(ctr,:) = [k,j];
       ctr=ctr+1;
    end
end
...
%c  = corr(tensorproduct');
%c  = corr(superpos_code');
%c  = corr(circ_conv');
%c  = corr(catted');
% %pearson correlation
 %c  = corr(catted');
