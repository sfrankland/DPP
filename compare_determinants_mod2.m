...
%  for p=1:length(candidates);    
 ...
 %cur_list=randsample(m^2, randsample(20,1));
 
 
 for L=1:500;
 %cur_list=randsample(m^2, 2);
 cur_list=randsample(m^2, randsample(2:10,1));
 listlength(L)=length(cur_list);
...
%      %normalization
         %DG = output(sub_pre,:)';
         %DG = output(sub_pre,:)';
        lateral_connections = output([cur_list],:)'*output([cur_list], :);
        num = sum(output([cur_list],:))>0;
        det_norm(L) = sum (num ./ (sum(lateral_connections)+0.001));
        %det_norm(L) = sum(sum(output([cur_list],:)) ./ (sum(lateral_connections')+0.001));
        %dets(L) = det(cov(output([cur_list], :)'));
        dets(L) = det(cov(output([cur_list], :)'));
         %present_absent = sum (sum(output(cur_list,:)) ./ (sum(lateral_connections')+0.001));
         %out = output(cur_list,:)./ (sum(lateral_connections') + 0.0001);
         %t = output(cur_list, :)./ (sum(output(cur_list, :)+0.001));
         %det_norm(L)= sum(sum((t>theta)));
         %out = output(cur_list,:)./ (sum(lateral_connections') + 0.0001);
         %det_norm(L)=sum(sum(norm_out);
         %lateral_connections = (DG.*DG);
         %s_lat = sum(lateral_connections);
         
%          
 for j=1:500;
 idx = randsample(100,4);
 lateral_connections = output([idx],:)'*output([idx], :);
 det_norm(j) = sum(sum(output([idx],:)) ./ (sum(lateral_connections')+0.001));
 dets(j) = det(cov(state_mat([idx], :)'));
 end
         
                    
                    for j=1:1000;
                    idx=randsample(100,4);
                    kout_cov=(output(idx,:)'*output(idx,:)*output(idx,:)');
                    kout_det=(output(idx,:)'/ output(idx,:)*output(idx,:)');
                    outavg_cov(j)=sum(sum(kout));
                    outavg_det(j)=sum(sum(kout_det));
                    dets(j) = det(cov(state_mat([idx], :)'))
                    c=cov(state_mat(idx,:)');
                          cavg(j)=mean(mean(c));
                    end
...
for j=1:1000;
idx=randsample(100,4);
%hebbian numerator, post-projection.
kout_cov=(output(idx,:)'*output(idx,:)*output(idx,:)');
outavg_cov(j)=sum(sum(kout_cov));
%hebbian denominator, post-projection.
kout_det=(output(idx,:)'/ (output(idx,:)*output(idx,:)') + 0.001);
outavg_det(j)=sum(sum(kout_det));

%determinant pre projection.
dets(j) = det(cov(state_mat([idx], :)'));
c=cov(state_mat(idx,:)');
cavg(j)=mean(mean(c));
end
         ...
         %det_norm_out(L) = sum((output/ (sum(lateral_connections)) + 0.001));
...
%         %norm_out = (output(list, :)*weights_layer2')./(s_lat+0.001);
     ...
     %[det_norm(L)] = sum(sum(out'));
%         sub_pre = [sub_pre list(bres)];
     %det_laplace(L) = Laplace(Itemp_orig);
      %det_hipp_prod(L) = prod(diag(Itemp_hipp));
      %det_hipp_sum(L) = sum(diag(Itemp_hipp));
    %  det_mat(L) = det(triu(Itemp_orig));
    %  det_mat2(L) = det(Itemp_orig);  
      
      det_mat(L) = det(triu(zscore(Itemp_orig)));
      %det_mat(L) = det(zscore(Itemp_orig));     
      
     % det_sum(L) = sum(diag(Itemp_orig));
...
%det_norm_prod(L,p) = prod(diag(triu(out)));
  ...
  avg_corr_mat = corr(det_norm', det_mat', 'type', 'Spearman');
  %avg_corr_mat(L) = corr(det_hipp_prod(L,:)', det_mat(L,:)', 'type', 'pearson');
%    [a1,b1]=max(det_hipp_prod);
%    [a2,b2]=max(det_mat2);
%    max_equal = b1==b2;
  %avg_corr_sum = corr(det_hipp_sum', det_sum');
